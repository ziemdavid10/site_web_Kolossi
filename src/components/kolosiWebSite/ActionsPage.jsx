...
const ActionsPage = () => {
  ...
  const actions = [
    ...
    {
      icon: <Target className="w-16 h-16" style={{color: '#bc00ac'}} />,
    ...
    {
      icon: <Users className="w-16 h-16" style={{color: '#8a0080'}} />,
      title: "Réseautage",
    ...
    <div className="min-h-screen bg-white pt-24 pb-16">
      <div className="px-4 sm:px-6 md:px-12 max-w-6xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h1 className="text-4xl md:text-5xl font-bold mb-4" style={{color: '#8a0080'}}>Nos Actions</h1>
          <p className="text-slate-600 text-lg max-w-2xl mx-auto">Découvrez en détail comment nous accompagnons les jeunes atteints de drépanocytose au Cameroun.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {actions.map((action, index) => (
            <div
              key={index}
              className="bg-gradient-to-br from-slate-50 to-white p-8 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 border-2 border-slate-200 hover:border-purple-300"
            >
              <div className="mb-6">{action.icon}</div>
              <h3 className="text-2xl font-bold mb-3" style={{color: '#8a0080'}}>{action.title}</h3>
              <p className="text-slate-600 mb-4 leading-relaxed">{action.details}</p>
              <div className="pt-4 border-t border-slate-200">
                <p className="text-sm text-slate-500 italic">{action.desc}</p>
              </div>
            </div>
          ))}
        </div>
